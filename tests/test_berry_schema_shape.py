from berry.schema import UserQL, PostQL, PostCommentQL, schema as berry_strawberry_schema
...
def test_userql_fields_shape():
    ...
    assert {'id','name','email','is_admin','created_at','posts','post_comments','post_agg','new_posts','other_users','bloggers'} <= f


def test_postql_fields_shape():
    f = set(PostQL.__berry_fields__.keys())
    assert {'id','title','content','author_id','created_at','author','post_comments','post_comments_agg','last_post_comment'} <= f
