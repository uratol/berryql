...
async def test_upsert_callbacks_create(db_session, populated_db):
    ...
    mutation = (
        "mutation($payload: PostQLInput!) { upsert_post(payload: $payload) { id title author_id } }"
    )
    variables = {
...
async def test_upsert_callbacks_update(db_session, populated_db):
    ...
    # First create
    create = "mutation($p: PostQLInput!) { upsert_post(payload: $p) { id title } }"
    ...
    pid = int(res1.data["upsert_post"]["id"])

    # Update: title change only
    upd = "mutation($p: PostQLInput!) { upsert_post(payload: $p) { id title } }"
    ...
    title = res2.data["upsert_post"]["title"]
