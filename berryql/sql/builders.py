...
class RelationSQLBuilders:
    ...
    def build_single_relation_object_from_child_fk(
        self,
        *,
        adapter,
        parent_model_cls,
        ...
        """Build a single related object using child->parent FK correlation.

        Applies where/default_where, filter args, ordering (order_multi -> order_by -> id), and limit 1.
        Returns an adapter-specific JSON expression for MSSQL, or a correlated JSON object subquery for others.
        """
        cols = projected_columns or ['id']
        mssql_mode = getattr(adapter, 'name', '') == 'mssql'
        child_table = child_model_cls.__tablename__
        parent_table = parent_model_cls.__tablename__
        if mssql_mode:
            # WHERE correlation and filters
            where_parts: list[str] = [
                f"[{child_table}].[{fk_child_to_parent_col.name}] = [{parent_table}].[id]"
            ]
            where_parts = self._mssql_where_from_value(
                where_parts,
                child_model_cls,
            ...
            order_clause = adapter.build_order_clause(
                ...
                self._effective_order_dir(rel_cfg),
                self.registry._normalize_order_multi_values(rel_cfg.get('order_multi') or []),
            )
            if not order_clause:
                # Fallback to id
                order_clause = adapter.build_order_clause(child_model_cls, child_table, 'id', 'asc', None)
            col_list = ', '.join([f"[{child_table}].[{c}]" for c in cols])
            raw = (
                f"ISNULL((SELECT TOP 1 {col_list} FROM {child_table} WHERE {where_sql} "
                f"ORDER BY {order_clause} FOR JSON PATH, WITHOUT_ARRAY_WRAPPER), 'null')"
            )
            from sqlalchemy import text as _text
            return _text(raw)
        # Non-MSSQL: correlated select with order and limit 1
        inner_cols = [getattr(child_model_cls, c) for c in cols]
        sel = (
            select(*inner_cols)
            .select_from(child_model_cls)
        ...
        sel = self._apply_ordering_sqla(
            ...
            order_dir=rel_cfg.get('order_dir'),
            order_multi=self.registry._normalize_order_multi_values(rel_cfg.get('order_multi') or []),
            dir_value_fn=self.registry._dir_value if hasattr(self.registry, '_dir_value') else None,
            default_dir_for_multi='asc',
            fallback_id=True,
        )
        sel = sel.limit(1)
        limited = sel.subquery()
        obj_expr = json_object_fn(*self._json_row_args_from_subq(limited, cols))
        query = select(obj_expr).select_from(limited).correlate(parent_model_cls)
        try:
            return query.scalar_subquery()
